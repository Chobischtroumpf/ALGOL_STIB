...
public class DVisualizer extends JFrame implements GLEventListener {
    ...
    private void calculateBounds() {
        ...
        minLat = Math.min(startStop.getLatitude(), endStop.getLatitude());
        maxLat = Math.max(startStop.getLatitude(), endStop.getLatitude());
        minLon = Math.min(startStop.getLongitude(), endStop.getLongitude());
        maxLon = Math.max(startStop.getLongitude(), endStop.getLongitude());
        ...
        for (Transition transition : finalPath) {
            ...
            if (fromStop != null) {
                minLat = Math.min(minLat, fromStop.getLatitude());
                maxLat = Math.max(maxLat, fromStop.getLatitude());
                minLon = Math.min(minLon, fromStop.getLongitude());
                maxLon = Math.max(maxLon, fromStop.getLongitude());
            }

            if (toStop != null) {
                minLat = Math.min(minLat, toStop.getLatitude());
                maxLat = Math.max(maxLat, toStop.getLatitude());
                minLon = Math.min(minLon, toStop.getLongitude());
                maxLon = Math.max(maxLon, toStop.getLongitude());
            }
        }
    ...
    public void display(@NotNull GLAutoDrawable drawable) {
        ...
        for (Stop stop : stops.values()) {
            ...
            if (pathIntermediateStops.contains(stop.getStopId()) ||
                    stop.getStopId().equals(recorder.startStopId) ||
                    stop.getStopId().equals(recorder.endStopId) ||
                    currentExploredStops.contains(stop.getStopId())) {
                continue;
            }

            float[] pos = geoToScreen(stop.getLatitude(), stop.getLongitude(), width, height);
        ...
        if (showPath) {
            ...
            for (Transition transition : finalPath) {
                ...
                if (fromStop != null && toStop != null) {
                    float[] fromPos = geoToScreen(fromStop.getLatitude(), fromStop.getLongitude(), width, height);
                    float[] toPos = geoToScreen(toStop.getLatitude(), toStop.getLongitude(), width, height);
        ...
        float[] startPos = geoToScreen(startStop.getLatitude(), startStop.getLongitude(), width, height);
        gl.glVertex2f(startPos[0], startPos[1]);

        // End stop
        gl.glColor4f(0.8f, 0.0f, 0.0f, 1.0f); // Red
        float[] endPos = geoToScreen(endStop.getLatitude(), endStop.getLongitude(), width, height);
        ...
        textRenderer.draw("Start Stop: " + startStop.getName(), legendX, legendY);
        textRenderer.draw("End Stop: " + endStop.getName(), legendX, legendY + lineHeight);
